...
- `claude-code` itself is configured declaratively in `home/default.nix` — plan-mode default, read-only Bash allowlist, MCP servers, and `diffity` skills wired from a pinned GitHub fetch. Prefer changing Claude Code's global config there rather than editing `~/.claude/settings.json` directly.
